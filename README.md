...
- в модель `Product` добавлено поле `is_published` (`BooleanField`, по умолчанию `False`);
- в модель `Product` добавлено поле `owner` (`ForeignKey` на пользователя);
- добавлено кастомное право `can_unpublish_product` для отмены публикации продукта;
- создана группа «Модератор продуктов» командой `create_moderator_group`;
- модераторам назначены права `can_unpublish_product` и `delete_product`;
- на главной странице отображаются только опубликованные продукты;
- при создании продукта поле `owner` автоматически заполняется текущим пользователем;
- редактирование и удаление доступны владельцу и модератору (`ProductOwnerOrModeratorMixin`);
- отмена публикации доступна пользователям с правом `can_unpublish_product` (`ProductUnpublishView`);
- кнопки редактирования, удаления и снятия с публикации отображаются в шаблоне по правам.
- настроен Redis как брокер кеширования (`CACHES` в `config/settings.py`);
- для страницы товара подключено кеширование контроллера через `cache_page` в `catalog/urls.py` (TTL 15 минут);
- добавлена сервисная функция `get_products_by_category` в `catalog/services.py`;
- реализована страница продуктов по категории: `/categories/<id>/`;
- для списка продуктов категории используется низкоуровневое кеширование с ключом `category_{id}` (TTL 1 час).
...
- `/products/<id>/delete/` — удаление продукта (владелец или модератор);
- `/products/<id>/unpublish/` — снятие продукта с публикации (модератор);
- `/categories/<id>/` — список опубликованных товаров категории;
...
- `templates/catalog/category_products.html` — товары выбранной категории;
...
1. Установить и запустить Redis (по умолчанию `127.0.0.1:6379`).
...
- `http://127.0.0.1:8000/categories/1/` — товары категории с id 1;
...
- `python manage.py create_moderator_group` — создание группы «Модератор продуктов».
